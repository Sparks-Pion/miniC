...
/*Expressions*/
Exp:      Exp ASSIGNOP Exp  {$$=new NAssignment("ASSIGNOP",*$1,ASSIGNOP,*$3);$$->line=yylineno;}//$$结点type_id空置未用，正好存放运算符
        
        | Exp PLUSASS Exp   {$$=new NAssignment("PLUSASS",*$1,PLUSASS,*$3);$$->line=yylineno;}//复合赋值运算
        | Exp MINUSASS Exp  {$$=new NAssignment("MINUSASS",*$1,MINUSASS,*$3);$$->line=yylineno;}
        | Exp STARASS Exp   {$$=new NAssignment("STARASS",*$1,STARASS,*$3);$$->line=yylineno;}
        | Exp DIVASS Exp    {$$=new NAssignment("DIVASS",*$1,DIVASS,*$3);$$->line=yylineno;}

        | PLUSPLUS Exp %prec UPLUSPLUS   {$$=new NSingleOperator("PLUSPLUS-",PLUSPLUS,*$2);$$->line=yylineno;}//这里利用BISON %prec表示和UMINUS同优先级
        | MINUSMINUS Exp %prec UMINUSMINUS   {$$=new NSingleOperator("MINUSMINUS-",MINUSMINUS,*$2);$$->line=yylineno;}//这里利用BISON %prec表示和UMINUS同优先级
        | Exp PLUSPLUS   {$$=new NSingleOperator("-PLUSPLUS",PLUSPLUS,*$1);$$->line=yylineno;}//这里利用BISON %prec表示和UMINUS同优先级
        | Exp MINUSMINUS  {$$=new NSingleOperator("-MINUSMINUS",MINUSMINUS,*$1);$$->line=yylineno;}//这里利用BISON %prec表示和UMINUS同优先级

        | Exp AND Exp   {$$=new NBinaryOperator("AND",*$1,AND,*$3);$$->line=yylineno;}
        | Exp OR Exp    {$$=new NBinaryOperator("OR",*$1,OR,*$3);$$->line=yylineno;}
        | Exp RELOP Exp {$$=new NBinaryOperator(*(new std::string("RELOP"))+$2,*$1,RELOP,*$3);$$->line=yylineno;}  //词法分析关系运算符号自身值保存在$2中
        | Exp PLUS Exp  {$$=new NBinaryOperator("PLUS",*$1,PLUS,*$3);$$->line=yylineno;}
        | Exp MINUS Exp {$$=new NBinaryOperator("MINUS",*$1,MINUS,*$3);$$->line=yylineno;}
        | Exp STAR Exp  {$$=new NBinaryOperator("STAR",*$1,STAR,*$3);$$->line=yylineno;}
        | Exp DIV Exp   {$$=new NBinaryOperator("DIV",*$1,DIV,*$3);$$->line=yylineno;}
        
        | Exp MOD Exp   {$$=new NBinaryOperator("MOD",*$1,MOD,*$3);$$->line=yylineno;}
        
        | LP Exp RP     {$$=new NParenOperator(*$2);$$->line=yylineno;}
        | MINUS Exp %prec UMINUS   {$$=new NSingleOperator("MINUS-",MINUS,*$2);$$->line=yylineno;}//这里利用BISON %prec表示和UMINUS同优先级 相当于虚拟出一个运算符
        | NOT Exp       {$$=new NSingleOperator("NOT-",NOT,*$2);$$->line=yylineno;}
